...
- Validaciones: `SAFE_REPO_RE` (`owner/repo`), `SAFE_REVIEW_EVENTS`
  ...
  `VALID_LABEL_RE`.
- Credencial: PAT bajo la key `github_token` del `KeychainManager`. El llavero del SO es la custodia
  preferida; si no está disponible, aplica el fallback local documentado en
  [`infrastructure/keychain`](../../infrastructure/keychain/README.md). Resolución aislada en
  `_resolveToken()` para permitir el flujo OAuth sin tocar las tools.
...
GitHub recibe repositorio, consultas y contenido de issues/PRs cuando se usa la integración. El
resultado puede incorporarse al prompt del LLM. Consulta el
[aviso de privacidad](../../docs/web/privacy.html) y autoriza únicamente los scopes necesarios.
