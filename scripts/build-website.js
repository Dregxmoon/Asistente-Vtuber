...
function guide(c) {
  ...
  return `<header class="page-heading container"><span class="eyebrow">KAORU / ${c.pages.guide}</span><h1>${c.pages.guide}</h1><p>${c.guideIntro}</p></header><div class="container document-layout">${toc}<article class="prose">${c.guideSections.map(([id, title, text], i) => `<section id="${id}"><h2>${i + 1}. ${title}</h2><p>${text}</p>${id === 'install' ? `<div class="code-block"><div class="code-heading"><span>Git / npm</span><button type="button" class="copy-button" data-copy="install-commands" data-copied="${c.copied}" data-error="${c.copyError}">${c.copy}</button></div><pre><code id="install-commands">${escape(commands)}</code></pre></div><p class="copy-status" role="status" aria-live="polite"></p><p>${c.installNote}</p><p class="inline-links"><a href="${repo}/blob/produccion/README.md">${c.installDocs} ↗</a><a href="${repo}/releases">${c.release} ↗</a></p>` : ''}</section>`).join('')}<p class="note"><a href="privacy.html">${c.pages.privacy}</a> · <a href="terms.html">${c.pages.terms}</a></p></article></div>`;
}
